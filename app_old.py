...
def analyze():
    # TODO: Save images locally then do a memoization
    data = request.get_json()
    text = str(data)
    ...
    for address in addresses:
        print("TRY ~~ "+address)
        try: loc = address_to_gps(address)
        except: print("Failed:"+address)
        gps = loc['gps']
        images = []
        resolution = '600x400'
        fov = '180'
        for i in range(len(loc['headings'])):
            ...
            try: response = requests.get(image_url, stream = True)
            except: print("Failed2:"+address)
        ...
        response = {'address':address,'images':images}
